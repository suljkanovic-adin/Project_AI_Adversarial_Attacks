...
2. **Several fixes have been made** in order to carry out the project properly:
   ...
   - **Sub-item 3**: In our case, the ART library used had some issues, so we were supposed to change [this line] to `targets = self._set_targets(x, y, False)` and add `target = np.expand_dims(target, axis=1)` after [this line] in the `pgd_attack_art.py` file.
