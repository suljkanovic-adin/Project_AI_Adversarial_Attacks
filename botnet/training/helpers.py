...
def create_DNN(units, input_dim_param, lr_param):
    network = Sequential()
    network.add(Input(shape=(756,)))  # Correct shape definition
    print(units)
    ...
    network.add(Dense(units=1))  # Output layer
    network.add(Activation('sigmoid'))
    sgd = Adam(learning_rate=lr_param)  # Use 'learning_rate' instead of 'lr'
...
def save_train_data(nn, x_test_array, y_test_array, metrics_path, adv_candidates_path):
    ...
    # Save adversarial candidates
    save_adv_candidates(x_test_array, y_test_array, predictions, adv_candidates_path)
...
def read_min_max(min_file, max_file):
    with open(min_file, 'r') as f:
        mins = f.read().strip().replace(' ', '').split(',')
        min_features = [float(i) for i in mins]

    with open(max_file, 'r') as f:
        maxs = f.read().strip().replace(' ', '').split(',')
        max_features = [float(i) for i in maxs]
...
def get_processing_data(config):
    ...
    eq_min_max = np.load(config["eq_min_max"])
